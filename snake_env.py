...
class Game:
    WHITE = (255, 255, 255)
    BLACK = (0, 0, 0)
    def __init__(self,Width=1200,Height=1200,tick_rate=60):
        pygame.init()
        self.block_size = 16
        ...
        self.Space_height = len(self.wall.map) - 2
        self.Space_width  = len(self.wall.map[0]) - 2
        self.nS = 20
        self.nA = 3
        self.reset()
    ...
    def head_hit_wall(self,position=None):
        ...
        if (position.y >= self.Space_height + 1 or
            position.x >= self.Space_width  + 1 or
